...
function BlogPage() {
  ...
  const [categories, setCategories] = useState([
    ...
    { id: 'kenyan-culture', name: 'Kenyan Culture', count: 0 },
    { id: 'ambassador-stories', name: 'Ambassador Stories', count: 0 },
    { id: 'tourism-updates', name: 'Tourism Updates', count: 0 }
  ]);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // Update category counts based on tags
  ...
  const transformPostData = (post) => ({
    ...
    category: post.tags?.[0] || 'Travel Tips',
    slug: post.slug
  });

  // Update category counts from fetched posts
  const updateCategoryCounts = (posts) => {
    const counts = {};
    posts.forEach(post => {
      const tag = post.tags?.[0]?.toLowerCase().replace(/\s+/g, '-') || 'travel-tips';
  ...
  const loadDemoData = () => {
    ...
    setFeaturedPost(transformPostData({
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8 text-center">
          ...
          {error && (
            <div className="mt-8 bg-red-500/20 border border-red-500/50 rounded-lg p-4 text-white">
            ...
            <div className="bg-gradient-to-r from-brand-green to-camp-600 rounded-2xl overflow-hidden shadow-2xl">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                <div className="relative h-64 lg:h-auto">
                  <OptimizedImage src={featuredPost.image} alt={featuredPost.title} className="w-full h-full object-cover" widths={[640, 960, 1280]} sizes="(min-width: 1024px) 50vw, 100vw" quality={75} />
              ...
              {filteredPosts.map((post, index) => (
                <article key={index} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                  <div className="relative h-48">
                    <OptimizedImage src={post.image} alt={post.title} className="w-full h-full object-cover" widths={[480, 640, 768, 1024]} sizes="(min-width: 1024px) 33vw, 100vw" quality={75} />
      ...
      {!loading && filteredPosts.length === 0 && (
        <section className="py-20">
          <div className="mx-auto max-w-7xl px-6 lg:px-8 text-center">
            <h2 className="text-3xl font-bold text-forest-900 mb-4">No Articles Found</h2>
            <p className="text-lg text-forest-700">Check back later for new content or try a different category.</p>
          </div>
        </section>
      )}
